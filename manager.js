...
function renderInventoryReport() {
    let inventorytbody = document.getElementById("inventorytbody");
    inventorytbody.innerHTML = ''; // Clear the table

    dvds.forEach(dvd => {
        let availableStock = dvd.quantity; // Assuming `quantity` stores available stock
        let rentedDvds = notifications.filter(n => n.dvdName === dvd.dvdName && n.status === 'Accepted').length;
        
        let row = document.createElement("tr");
        row.innerHTML = `
            <td>Cinema Corner</td> <!-- Shop name -->
            <td>${dvd.category}</td>
            <td>${availableStock + rentedDvds}</td> <!-- Total DVDs -->
            <td>${availableStock}</td> <!-- Available stock -->
        `;
...
function renderCustomerReport() {
    let customertbody = document.getElementById("customertbody");
    customertbody.innerHTML = ''; // Clear the table

    // Get total customers
    let totalCustomers = users.length;

    // Calculate active customers (those with 'Accepted' status in notifications)
    let activeCustomers = notifications.filter(notification => notification.status === 'Accepted').map(notification => notification.userName);
    let uniqueActiveCustomers = [...new Set(activeCustomers)]; // Unique list of active customers

    // Create a new row with the report data
    let row = document.createElement("tr");
    row.innerHTML = `
        <td>Cinema Corner</td> <!-- Shop name -->
        <td>${totalCustomers}</td> <!-- Total customers -->
        <td>${uniqueActiveCustomers.length}</td> <!-- Active customers -->
    `;
...
function renderRentalReport() {
    let rentaltbody = document.getElementById("rentaltbody");
    rentaltbody.innerHTML = ''; // Clear the table

    notifications.forEach(notification => {
        if (notification.status === 'Accepted') {
            let isOverdue = false;
            let dvdCategory = 'Unknown'; // Default category
            let returnDate = notification.returnDate || 'N/A';

            // Check if returnDate is valid and compare with current date
            if (returnDate !== 'N/A') {
                isOverdue = new Date(returnDate) < new Date();
            }

            // Find the DVD details based on the dvdName
            let dvd = dvds.find(dvd => dvd.dvdName === notification.dvdName);
            if (dvd) {
                dvdCategory = dvd.category; // Get the category from the DVD object
            }

            // Create the row for the rental report
            let row = document.createElement("tr");
            row.innerHTML = `
                <td>Cinema Corner</td> <!-- Shop name -->
                <td>${dvdCategory}</td> <!-- DVD category -->
                <td>${notification.dvdName}</td>
                <td>${isOverdue ? "Yes" : "No"}</td> <!-- Overdue status -->
            `;
            rentaltbody.appendChild(row);
        } 
    });
}
